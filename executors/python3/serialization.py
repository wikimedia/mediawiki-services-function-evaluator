...
def deserialize(ZObject):
    ...
    if deserializer is None:
        raise exceptions.EvaluatorError('Could not deserialize input ZObject type: {}'.format(ZID))
...
def serialize(py_object, expected_type):
    ...
    if ZID is not None and serializer is not None:
        try:
            ...
            raise
            pass
    raise exceptions.EvaluatorError('Could not serialize input python object: {}'.format(repr(py_object)))
